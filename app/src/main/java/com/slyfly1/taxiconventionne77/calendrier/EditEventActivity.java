...
import com.slyfly1.taxiconventionne77.enregistrementclient.DBClientHelper;
...
import static com.slyfly1.taxiconventionne77.calendrier.SettingsActivity.ReminderAmount;
import static com.slyfly1.taxiconventionne77.calendrier.SettingsActivity.ReminderDate;
...
public class EditEventActivity extends AppCompatActivity implements DatePickerDialog.OnDateSetListener,
    ...
    private EditText eventNameET, noteET, locationText,arriverText,editText21,montantText,recherchertelephone;
    private Spinner spinnerCpam,spinnerMontant;
    private TextView title;
    private ImageButton startDateButton, endDateButton, startTimeButton, endTimeButton,imagebutton12;
    ...
    private String currentDateString, start, end, eventName, repeatType, untilDate, repeatCount,
            note, location,location1,arriver, link, repeatFrequency, durationType, startDB, endDB, realStart,
            seriText, amountSP,prenom,spinercpam,spinnerreglement,montant;
...
montantText=findViewById(R.id.editMontant);
spinnerCpam=findViewById(R.id.spinnercpam);
spinnerMontant=findViewById(R.id.spinnerpayement);
        ...
        recherchertelephone=findViewById(R.id.rechtelephone);
arriverText =findViewById(R.id.textarriver);
editText21=findViewById(R.id.editText2);
...
Cursor res=DB.recherchetelephonedata(b);
        while(res.moveToNext()){
            eventNameET.setText(res.getString(0));
            editText21.setText(res.getString(1));
            noteET.setText(res.getString(2));
            locationText.setText(res.getString(3));
            arriverText.setText(res.getString(4));
...
imagebutton12.setOnClickListener(new View.OnClickListener() {
    ...
    public void onClick(View view) {
        if (ActivityCompat.checkSelfPermission(EditEventActivity.this, Manifest.permission.CALL_PHONE) != PackageManager.PERMISSION_GRANTED) {
            ...
            Intent appel = new Intent(Intent.ACTION_DIAL, Uri.parse("tel:"+noteET.getText().toString()));
            startActivity(appel);
        ...
        if (getIntent().getStringExtra("EDIT") != null) {
            ...
            arriver=getIntent().getStringExtra("ARRIVER");
            location=getIntent().getStringExtra("DEPART");
            prenom=getIntent().getStringExtra("PRENOM");
            spinercpam=getIntent().getStringExtra("CPAM");
            spinnerreglement=getIntent().getStringExtra("MODEDEPAYEMENT");
            montant=getIntent().getStringExtra("MONTANT");
            DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm");

            ArrayList<String> area = new ArrayList<>();
//add values in area arrayList
            spinnerCpam.setAdapter(new ArrayAdapter<String>(this
                    , android.R.layout.simple_list_item_1, area));
            spinnerMontant.setAdapter(new ArrayAdapter<String>(this
                    , android.R.layout.simple_list_item_1, area));
            ...
            editText21.setText(prenom);
            locationText.setText(location);
            arriverText.setText(arriver);
            montantText.setText(montant);

        if(spinercpam.equals("Non")) {
                ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(this,
                        R.array.defaultcpam, android.R.layout.simple_spinner_item);
                adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
                spinnerCpam.setAdapter(adapter);


            }else{   ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(this,
                R.array.defaultcpam1, android.R.layout.simple_spinner_item);
            adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
            spinnerCpam.setAdapter(adapter);
            ...
            ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(this,
                    R.array.defaultespece, android.R.layout.simple_spinner_item);
            adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
            spinnerMontant.setAdapter(adapter);

            if (spinnerreglement.equals("CPAM")){
                spinnerMontant.setSelection(4);
            }else if (spinnerreglement.equals("Espèce")){
                spinnerMontant.setSelection(1);
            }else if (spinnerreglement.equals("Chèque")){
                spinnerMontant.setSelection(3);
            }else if (spinnerreglement.equals("CB")){
                spinnerMontant.setSelection(2);
            }else if(spinnerreglement.equals("")){
                spinnerMontant.setSelection(0);
            }


            title.setText("MODIFIER");
            sTimeSet = true;
            eTimeSet = true;
            findRow();
            if (note != null) {
        ...
        saveButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(final View view) {
                ...
                location1=locationText.getText().toString();
                arriver=arriverText.getText().toString();
                prenom=editText21.getText().toString();
                spinercpam=spinnerCpam.getSelectedItem().toString();
                spinnerreglement=spinnerMontant.getSelectedItem().toString();
                ...
                if (!dateBoundValid()) {
                    ...
                    builder.setPositiveButton("Only this event", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            insertToCV(eventName, start, end, note, location1,arriver,prenom,spinercpam,spinnerreglement,montant);
                ...
                Intent intent= new Intent(EditEventActivity.this,MainActivitycalendrier.class);
    ...
    public Date repeatEventCreator(MyDate buffer, Date until, View view) {
        ...
        if ((durationType.equals("Until") && start.getTime().before(until) &&
            ...
            cv.put(EventDB.Event.COLUMN_ARRIVER,arriverText.getText().toString());
            cv.put(EventDB.Event.COLUMN_PRENOM,editText21.getText().toString());
            cv.put(EventDB.Event.COLUMN_CPAM,spinercpam=spinnerCpam.getSelectedItem().toString());
            cv.put(EventDB.Event.COLUMN_MODEPAYEMENT,spinnerreglement=spinnerMontant.getSelectedItem().toString());
            cv.put(EventDB.Event.COLUMN_MONTANT,montant=montantText.getText().toString());
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                if (type.equals("Delete")) {
                    ...
                    insertToCV(eventName, newStartDate, newEndDate, note, location1,arriver,prenom,spinercpam,spinnerreglement,montant);
                    cv.put(EventDB.Event.COLUMN_SERI, SERI);
                    updateRow(view, id);
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                if (type.equals("Delete")) {
                    ...
                    String newStartDate = date.split(" ")[0] + " " + start.split(" ")[1];
                    String newEndDate = date.split(" ")[0] + " " + end.split(" ")[1];
                    insertToCV(eventName, newStartDate, newEndDate, note,location,arriver,prenom,spinercpam,spinnerreglement,montant);
                    cv.put(EventDB.Event.COLUMN_SERI, SERI);
                    updateRow(view, id);
                    if (reminded && deleteAll) {
                        mDatabase.delete(EventDB.Event.REMINDER_TABLE_NAME,
    ...
    public void insertToCV(String eventName, String start, String end, String note,String location1,String arriver,String prenom,String spinercpam,String spinnerreglement,String montant) {
        cv = new ContentValues();
        cv.put(EventDB.Event.COLUMN_NAME, eventName);
        cv.put(EventDB.Event.COLUMN_START, start);
        cv.put(EventDB.Event.COLUMN_END, end);
        cv.put(EventDB.Event.COLUMN_NOTE, note);
        cv.put(EventDB.Event.COLUMN_LOCATION,location1);
        cv.put(EventDB.Event.COLUMN_ARRIVER,arriver);
        cv.put(EventDB.Event.COLUMN_PRENOM,prenom);
        cv.put(EventDB.Event.COLUMN_CPAM,spinercpam);
        cv.put(EventDB.Event.COLUMN_MODEPAYEMENT,spinnerreglement);
        cv.put(EventDB.Event.COLUMN_MONTANT,montant);
      //  if (located) {
       //     cv.put(EventDB.Event.COLUMN_LOCATION, location);
         //   cv.put(EventDB.Event.COLUMN_LOCATION_LINK, link);
        //}
    }
public void insertToClient(String eventName,String prenom,String location1,String arriver,String telephone){

}
    public void updateRow(View view, int id) {
        mDatabase.update(EventDB.Event.TABLE_NAME, cv, EventDB.Event.COLUMN_ID + "=" + id,
                null);
        Snackbar mySnackbar = Snackbar.make(view, "Mis a jours", Snackbar.LENGTH_SHORT);
        mySnackbar.show();
        saved = true;
        eventNameET.getText().clear();
        editText21.getText().clear();
...
if (checkBox.isChecked()){
    ...
    String nomTXT = eventNameET.getText().toString();
    String prenomTXT = editText21.getText().toString();
    String telephoneTXT = noteET.getText().toString();
    String adresseTXT = locationText.getText().toString();
    String arriverTXT = arriverText.getText().toString();
    ...
    Boolean checkinsertdata = DB.insertuserdata(nomTXT, prenomTXT, telephoneTXT,adresseTXT,arriverTXT);
        ...
        if(checkinsertdata==true){
            ...
            Toast.makeText(EditEventActivity.this,  "Nouveau client Ajouté", Toast.LENGTH_SHORT).show();

        }
        else{
            Toast.makeText(EditEventActivity.this, "Non ajouté", Toast.LENGTH_SHORT).show();
        }}
        eventNameET.getText().clear();
        editText21.getText().clear();
        locationText.getText().clear();
